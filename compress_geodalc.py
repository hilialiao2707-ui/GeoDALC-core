#coding:utf8
"""GeoDALC activation-guided low-rank compression entry point.

This file preserves the experiment-compatible SVD-LLM compression path and
adds support for geoscience-mixed calibration through ``utils.data_utils``.
"""
...
def profle_svdllm_low_resource(model_name, model, calib_loader, dev):
    ...
    cache = {'i': 0, 'attention_mask': None, "position_ids": None, 'inps': None}
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            inp_cpu = inp.cpu()
            if cache['inps'] is None:
                cache['inps'] = torch.zeros((len(calib_loader), inp_cpu.shape[1], inp_cpu.shape[2]), dtype=inp_cpu.dtype)
            cache['inps'][cache['i']] = inp_cpu
            cache['i'] += 1
    ...
    for i in tqdm(range(len(layers))):
        ...
        for j in range(inps.shape[0]):
            if "opt" not in model_name:
                outs[j] = layer(inps[j].unsqueeze(0).to(dev), attention_mask=attention_masks[j].unsqueeze(0).to(dev), position_ids=position_ids[j].unsqueeze(0).to(dev))[0]
            else:
                outs[j] = layer(inps[j].unsqueeze(0).to(dev), attention_mask=attention_masks[j].unsqueeze(0).to(dev))[0]
        for h in handles:
            h.remove()
        layer = layer.cpu()
        for name in subset:
...
def whitening(model_name, model, profiling_mat, ratio, dev):
    ...
    for i in tqdm(range(len(layers))):
        print(f"[whitening] layer={i} start", flush=True)
        layer = layers[i]
        subset = find_layers(layer)
        print(f"[whitening] layer={i} modules={list(subset.keys())}", flush=True)
        #### Replace Attn, MLP ####
        ...
        for name in subset:
            ...
            if 'opt' in model_name:
                ...
                if "q_proj" in name:
                    ...
                    layer.mlp = svd_mlp
            print(f"[whitening] layer={i} module={name} done", flush=True)
        ...
        print(f"[whitening] layer={i} done", flush=True)
        del layer
        torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='meta-llama/Llama-2-7b-hf', help='Hugging Face model identifier or local model directory.')
    parser.add_argument('--model_path', type=str, default=None, help='local compressed model path or whitening information path')
    parser.add_argument('--ratio', type=float, default=0.2, help='Compression ratio in (0,1). A value of 0.2 retains an internal factor budget of 0.8.')
    ...
    parser.add_argument('--dataset', type=str, default='wikitext2_geogpt20',help='Calibration source: wikitext2, geogpt, or wikitext2_geogpt{5,10,15,20,30}.')
    ...
    parser.add_argument('--eval_after_whitening_dataset', type=str, default=None, help='run ppl_eval on this dataset immediately after whitening and exit before save')
    parser.add_argument('--eval_after_whitening_seq_len', type=int, default=None, help='sequence length for immediate post-whitening ppl_eval')
    ...
    if args.step == 1:
        ...
        if args.profiling_mat_path is None:
            ...
            if args.save_path is not None:
                profiling_out = args.save_path + '/' + args.model.replace('/', '_').replace('-', '_') + '_profiling_'+ args.dataset + '_' + str(args.whitening_nsamples)  + '_' + str(args.seed)+ '.pt'
                try:
                    torch.save(profiling_mat, profiling_out)
                    print(f'Saved profiling matrix to {profiling_out}')
                except Exception as e:
                    print(f'Skip profiling_mat save due to write failure: {e}')
        else:
            profiling_mat = torch.load(args.profiling_mat_path)
        whitening(args.model, model, profiling_mat, args.ratio, args.DEV)
        if args.eval_after_whitening_dataset is not None:
            model.eval()
            model = model.float()
            model = model.to(args.DEV)
            ppl_eval(model, tokenizer, datasets=[args.eval_after_whitening_dataset], model_seq_len=(args.eval_after_whitening_seq_len if args.eval_after_whitening_seq_len is not None else args.model_seq_len), batch_size=1, device=args.DEV)
            raise SystemExit(0)
        if args.save_path is not None:
            torch.save(
        ...
        if args.model_path == "original":
            ...
            if args.lora is not None:
                ...
                torch.save({'model': model, 'tokenizer': tokenizer}, args.lora + '/merge.pt', _use_new_zipfile_serialization=False)
        model.eval()
        model = model.float()
        model = model.to(args.DEV)
        if args.step == 4:
